<div class="row">
    <h3 align="center" class="col-md-12 col-sm-12 col-xs-12"><a href="#"><strong>TẤT CẢ SẢN PHẨM</strong></a></h3>
    <div class="row" >
    ...
    <br />
 	<?php
		...
		if ($pg == '' || $pg == 1) {
			...
			$pg1 = ($pg - 1) * 16;
		$sql_date = "select * from product order by date desc limit $pg1, 16";
...
<?php
	if($pg == '' || $pg == 1)
		...
		echo '<li><a href="?view=all&page='.($pg - 1).'">&laquo;</a></li>';
	...
	for($i = 1; $i <= $page; $i++) {
		if($pg == $i)
			echo '<li class="active"><a href="?view=all&page='.$i.'"> '.$i.'</a></li>';
		else
			echo '<li><a href="?view=all&page='.$i.'"> '.$i.'</a></li>';
	...
	if($pg == $page)
		...
		echo '<li><a href="?view=all&page='.($pg + 1).'">&raquo;</a></li>';
?>
