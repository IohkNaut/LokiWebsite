...
// Data loading
$sql = "select product_name, product_size, quantity, price, discount, orders.date as date 
from orders join orders_detail on orders_detail.orders_id = orders.orders_id, product
where orders.status = 1 and month(orders.date) = '$month' and year(orders.date) = '$year' and product.product_id = orders_detail.product_id
order by date";
$run = mysql_query($sql);
...
$query = mysql_query("select sum(cost) as sum from orders 
where orders.status = 1 and month(orders.date) = '$month' and year(orders.date) = '$year'");
$sum = mysql_fetch_array($query);
$pdf->Cell(110);
$pdf->SetFont('DejaVu','',14);
$pdf->Cell(0,0,'Tổng doanh thu: '.number_format($sum['sum']).' vnđ',0, 1);
