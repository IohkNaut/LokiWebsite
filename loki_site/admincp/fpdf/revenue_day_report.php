...
where orders.status = 1 and date(orders.date) = '$day' and product.product_id = orders_detail.product_id
order by date";
$run = mysql_query($sql);
...
$query = mysql_query("select sum(cost) as sum from orders where orders.status = 1 and date(orders.date) = '$day'");
